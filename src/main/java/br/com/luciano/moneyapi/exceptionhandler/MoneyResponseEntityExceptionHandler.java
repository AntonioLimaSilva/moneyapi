package br.com.luciano.moneyapi.exceptionhandler;
...
public class MoneyResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	/**
	 * Capturando exception quando o usuário tentar excluir uma entidade não
	 * existente
	 * 
	 * @param ex
	 * @param request
	...
	public ResponseEntity<Object> handleInvalidGrantException(InvalidGrantException ex,
		...
		String mensagemUsuario = messageSource.getMessage("session-expirada", null, LocaleContextHolder.getLocale());
		String mensagemDesenvolvedor = ExceptionUtils.getRootCauseMessage(ex);
		List<Erro> erros = Arrays.asList(new Erro(mensagemUsuario, mensagemDesenvolvedor));
